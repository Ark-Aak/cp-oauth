<template>
    <el-card class="oauth-callback-card" shadow="never">
        <h1 class="oauth-callback-card__title">{{ $t('auth.login.clist_callback_title') }}</h1>

        <div v-if="loading" class="oauth-callback-card__status">
            <el-icon class="is-loading"><Loading /></el-icon>
            <span>{{ $t('auth.login.clist_callback_loading') }}</span>
        </div>
        ...
        <el-alert
            v-else
            :title="$t('auth.login.clist_callback_success')"
            type="success"
            show-icon
            :closable="false"
        />
...
const { t } = useI18n();
useHead({ title: () => `${t('auth.login.clist_callback_title')} - CP OAuth` });
...
async function finishClistLogin() {
    if (oauthError.value) {
    ...
    if (!oauthCode.value || !oauthState.value) {
        errorMessage.value = t('auth.login.clist_callback_invalid');
        loading.value = false;
        return;
    }
    ...
    try {
        const result = await $fetch<{
            ...
            token?: string;
            redirect?: string;
        }>('/api/auth/thirdparty/clist/callback', {
            method: 'POST',
            body: {
        ...
        if (result.mode === 'bind') {
            ElMessage.success(t('binding.clist_bind_success'));
        }
